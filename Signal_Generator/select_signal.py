...
def select():
    ...
    if select_signal == 'sine':
        signal = sig.sine(frequency=frequency)
    elif select_signal == 'cosine':
        signal = sig.cosine(frequency=frequency)
    elif select_signal == 'chirp':
        signal = sig.chrip_signal(
            frequency_start=f0, frequency_end=f1, method=method, vertex=vertex)
